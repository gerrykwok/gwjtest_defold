...
#include "tolua_fix.h"
//g #include "base/CCRef.h"
//g #include "LuaBasicConversions.h"
#include <stdlib.h>

//g using namespace cocos2d;
...
//g TOLUA_API int toluafix_pushusertype_ccobject(lua_State* L,
//g                                              int refid,
//g                                              int* p_refid,
//g                                              void* ptr,
//g                                              const char* type)
//g {
//g     if (ptr == NULL || p_refid == NULL)
//g     {
//g         lua_pushnil(L);
//g         return -1;
//g     }
//g     
//g     Ref* vPtr = static_cast<Ref*>(ptr);
//g     const char* vType = getLuaTypeName(vPtr, type);
//g 
//g     if (*p_refid == 0)
//g     {
//g         *p_refid = refid;
//g 
//g         lua_pushstring(L, TOLUA_REFID_PTR_MAPPING);
//g         lua_rawget(L, LUA_REGISTRYINDEX);                           /* stack: refid_ptr */
//g         lua_pushinteger(L, refid);                                  /* stack: refid_ptr refid */
//g         lua_pushlightuserdata(L, vPtr);                              /* stack: refid_ptr refid ptr */
//g 
//g         lua_rawset(L, -3);                  /* refid_ptr[refid] = ptr, stack: refid_ptr */
//g         lua_pop(L, 1);                                              /* stack: - */
//g 
//g         lua_pushstring(L, TOLUA_REFID_TYPE_MAPPING);
//g         lua_rawget(L, LUA_REGISTRYINDEX);                           /* stack: refid_type */
//g         lua_pushinteger(L, refid);                                  /* stack: refid_type refid */
//g         lua_pushstring(L, vType);                                    /* stack: refid_type refid type */
//g         lua_rawset(L, -3);                /* refid_type[refid] = type, stack: refid_type */
//g         lua_pop(L, 1);                                              /* stack: - */
//g 
//g         //printf("[LUA] push CCObject OK - refid: %d, ptr: %x, type: %s\n", *p_refid, (int)ptr, type);
//g     }
//g 
//g     tolua_pushusertype_and_addtoroot(L, vPtr, vType);
//g     
//g     return 0;
//g }
